...
tarot.post('/create', async (req: Request, res: Response) => {
    ...
    if (!reader) { return res.status(400).send('Invalid reader') }
...
tarot.post('/cash', async (req: Request, res: Response) => {
    ...
    if (!event || !event.staff_password_hash) { return res.status(400).send('Staff password not set') }
    const valid = await bcrypt.compare(staff_password, event.staff_password_hash)
    if (!valid) { return res.status(403).send('Invalid staff password') }
...
tarot.get('/readers', async (_req: Request, res: Response) => {
...
tarot.get('/slots/:reader_id/:package_type', async (req: Request, res: Response) => {
