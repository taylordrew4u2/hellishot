...
admin.post('/clear-all', async (_req, res) => {
    try {
...
admin.post('/seed-event', async (_req, res) => {
    try {
        ...
        const { data: event, error: eventError } = await supabaseAdmin
            ...
            .single();
        if (eventError)
            return res.status(500).json({ error: 'Failed to create event', details: eventError });
        ...
        if (blocksError)
            return res.status(500).json({ error: 'Failed to create blocks', details: blocksError });
